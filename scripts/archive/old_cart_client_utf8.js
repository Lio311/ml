...
export default function CartClient() {
    ...
    const vendorBuckets = useMemo(() => {
        ...
        cartItems.forEach(item => {
            ...
            if (!buckets[vId]) {
                buckets[vId] = {
                    id: vId,
                    name: item.vendorName || (vId === 'main' ? 'ÎöÎÉÎ¬Î¿ ÎöÎ¿Î®Î×ÎÖ' : 'ÎíÎñÎº ÎùÎÖÎªÎòÎáÎÖ'),
                    items: []
                };
            }
    ...
    const handleLoadSharedCart = () => {
        ...
        if (confirm("ÎñÎóÎòÎ£Îö ÎûÎò Î¬ÎùÎ£ÎÖÎú ÎÉÎ¬ ÎöÎíÎ£ ÎöÎáÎòÎøÎùÎÖ Î®Î£ÎÜ ÎæÎíÎ£ ÎöÎ×Î®ÎòÎ¬Îú. ÎöÎÉÎØ Î£ÎöÎ×Î®ÎÖÎÜ?")) {
            clearCart();
            sharedCart.forEach(item => {
                for (let k = 0; k < item.quantity; k++) {
                    addToCart(item, item.size, item.price, item.vendorId || 'main', item.vendorName || 'ÎöÎÉÎ¬Î¿ ÎöÎ¿Î®Î×ÎÖ');
                }
            });
            setSharedCart(null);
            router.replace('/cart');
    ...
    const handleShareCart = async () => {
        if (activeItems.length === 0) {
            toast.error("ÎöÎíÎ£ ÎöÎáÎòÎøÎùÎÖ Î¿ÎÖÎº ÎòÎ£ÎøÎƒ Î£ÎÉ ÎáÎÖÎ¬Îƒ Î£Î®Î¬Îú ÎÉÎòÎ¬Îò");
            return;
        }
        try {
            ...
            if (res.ok) {
                ...
                if (navigator.share) {
                    navigator.share({ title: 'ÎöÎíÎ£ Î®Î£ÎÖ Îæ-ml_tlv', text: 'ÎæÎáÎÖÎ¬ÎÖ ÎÉÎùÎ£Îö ÎíÎ£, Î×Îö ÎôÎóÎ¬ÎÜ?', url }).catch(console.error);
                } else {
                    navigator.clipboard.writeText(url).then(() => toast.success("ÎöÎºÎÖÎ®ÎòÎ¿ ÎöÎòÎóÎ¬Îº, Î®Î¬ÎñÎò ÎæÎøÎÖÎú!"));
                }
            } else {
                toast.error("Î®ÎÆÎÖÎÉÎö ÎæÎÖÎªÎÖÎ¿Î¬ ÎºÎÖÎ®ÎòÎ¿ Î®ÎÖÎ¬ÎòÎú");
            }
        } catch (e) {
            console.error(e);
            toast.error("Î®ÎÆÎÖÎÉÎö ÎæÎÖÎªÎÖÎ¿Î¬ ÎºÎÖÎ®ÎòÎ¿ Î®ÎÖÎ¬ÎòÎú");
    ...
    useEffect(() => {
        ...
        if (limits.allowed_users?.length > 0) {
            ...
            if (!userEmail || !limits.allowed_users.some(u => u.trim().toLowerCase() === userEmail.trim().toLowerCase())) {
                isValid = false;
                rejectReason = 'ÎöÎºÎòÎñÎòÎƒ Î×ÎòÎÆÎƒ: ÎÉÎÖÎáÎÜ ÎæÎ¿Î®ÎÖÎ×Î¬ ÎöÎ×Î®Î¬Î×Î®ÎÖÎØ';
            }
        }

        if (isValid && limits.min_cart_total && subtotal < limits.min_cart_total) {
            isValid = false;
            rejectReason = `ÎöÎºÎòÎñÎòÎƒ Î×ÎòÎÆÎƒ: ÎÉÎÖÎáÎÜ Î×ÎÆÎÖÎó Î£ÎíÎøÎòÎØ ${limits.min_cart_total} Ôé¬`;
        }
        ...
        if (isValid && hasItemFilters) {
            ...
            if (!hasMatch) {
                isValid = false;
                rejectReason = 'ÎöÎºÎòÎñÎòÎƒ Î£ÎÉ ÎùÎ£ ÎóÎ£ ÎñÎ¿ÎÖÎÿÎÖÎØ ÎÉÎ£Îö';
            }
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (limits.allowed_users?.length > 0) {
                    const userEmail = user?.primaryEmailAddress?.emailAddress;
                    if (!userEmail || !limits.allowed_users.some(u => u.trim().toLowerCase() === userEmail.trim().toLowerCase())) {
                        setCouponError('ÎöÎºÎòÎñÎòÎƒ ÎöÎûÎö ÎÉÎÖÎáÎò ÎûÎ×ÎÖÎƒ ÎóÎæÎòÎ¿ Î×Î®Î¬Î×Î® ÎûÎö');
                        setCoupon(null);
                        return;
                    }
                }
                ...
                if (limits.min_cart_total && subtotal < limits.min_cart_total) {
                    setCouponError(`ÎöÎºÎòÎñÎòÎƒ ÎÉÎÖÎáÎò Î×ÎÆÎÖÎó Î£ÎíÎøÎòÎØ ${limits.min_cart_total} Ôé¬`);
                    setCoupon(null);
                    return;
                }
                ...
                if (hasItemFilters) {
                    ...
                    if (!hasMatch) {
                        setCouponError('ÎöÎºÎòÎñÎòÎƒ ÎöÎûÎö ÎÉÎÖÎáÎò ÎùÎ£ ÎóÎ£ ÎñÎ¿ÎÖÎÿÎÖÎØ ÎæÎíÎ£ Î®Î£ÎÜ');
                        setCoupon(null);
                        return;
                    }
                }
                ...
                setCouponError('ÎºÎòÎô ÎºÎòÎñÎòÎƒ Î£ÎÉ Î¬ÎºÎÖÎƒ');
                setCoupon(null);
            }
        } catch (e) {
            console.error(e);
            setCouponError('Î®ÎÆÎÖÎÉÎö ÎæÎæÎôÎÖÎºÎ¬ ÎöÎºÎòÎñÎòÎƒ');
        } finally {
    ...
    const validatePhone = (phone) => {
        if (!phone) return "Î×ÎíÎñÎ¿ ÎÿÎ£ÎñÎòÎƒ ÎöÎòÎÉ Î®ÎôÎö ÎùÎòÎæÎö";
        const cleanPhone = phone.replace(/\D/g, '');
        if (!cleanPhone.startsWith('05')) return "Î×ÎíÎñÎ¿ ÎÿÎ£ÎñÎòÎƒ ÎªÎ¿ÎÖÎÜ Î£ÎöÎ¬ÎùÎÖÎ£ Îæ-05";
        if (cleanPhone.length !== 10) return "Î×ÎíÎñÎ¿ ÎÿÎ£ÎñÎòÎƒ ÎªÎ¿ÎÖÎÜ Î£ÎöÎÖÎòÎ¬ 10 ÎíÎñÎ¿ÎòÎ¬";
        return "";
    };
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const data = await res.json();
                toast.error(`Î®ÎÆÎÖÎÉÎö: ${data.error}`);
                setIsSubmitting(false);
            }
        } catch (e) {
            console.error(e);
            toast.error('ÎÉÎÖÎ¿ÎóÎö Î®ÎÆÎÖÎÉÎö. ÎæÎôÎòÎº ÎùÎÖÎæÎòÎ¿ Î£Î¿Î®Î¬.');
            setIsSubmitting(false);
        }
    };
    ...
    if (isSubmitting) {
        return (
            <div className="container py-20 text-center flex flex-col items-center justify-center min-h-[60vh]">
                ...
                <h2 className="text-2xl font-bold animate-pulse">Î×ÎóÎæÎô ÎÉÎ¬ ÎöÎöÎûÎ×ÎáÎö Î®Î£ÎÜ...</h2>
                <p className="text-gray-500 mt-2">Î¿ÎÆÎó ÎºÎÿÎƒ, ÎøÎ×ÎóÎÿ ÎíÎÖÎÖÎ×ÎáÎò.</p>
            </div>
        );
    }
            ...
            <div className="container py-20 text-center">
                {sharedCart && (
                    <div className="bg-blue-600 text-white p-4 rounded-xl mb-8 shadow-lg max-w-2xl mx-auto flex items-center justify-between animate-fade-in text-right" dir="rtl">
                        <div>
                            <h3 className="font-bold text-lg">ÎºÎÖÎæÎ£Î¬ ÎíÎ£ Î×Î®ÎòÎ¬Îú! ­ƒøì´©Å</h3>
                            <p className="text-sm opacity-90">ÎöÎíÎ£ Î×ÎøÎÖÎ£ {sharedCart.length} ÎñÎ¿ÎÖÎÿÎÖÎØ. Î¿ÎòÎªÎö Î£ÎÿÎóÎòÎƒ ÎòÎ£ÎöÎ×Î®ÎÖÎÜ?</p>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => { setSharedCart(null); router.replace('/cart'); }} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm transition">Î£ÎæÎÿÎ£</button>
                            <button onClick={handleLoadSharedCart} className="px-4 py-2 bg-white text-blue-600 font-bold rounded-lg text-sm hover:scale-105 transition shadow">ÎÿÎóÎƒ ÎíÎ£</button>
                        </div>
                    </div>
                )}
                <h1 className="text-3xl font-bold mb-4">ÎöÎóÎÆÎ£Îö Î®Î£ÎÜ Î¿ÎÖÎºÎö</h1>
                <p className="text-gray-500 mb-8">ÎæÎùÎ¿Î¬ Î£ÎöÎÆÎÖÎó Î£ÎøÎÉÎƒ Î¿ÎÖÎº... ÎæÎòÎÉÎò ÎáÎ×Î£ÎÉ ÎÉÎòÎ¬Îö</p>
                <Link href="/catalog" className="btn btn-primary">ÎùÎûÎ¿Îö Î£ÎºÎÿÎ£ÎòÎÆ</Link>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="container py-12">
                ...
                {lotteryMode?.active && (
                    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white p-4 rounded-xl mb-8 shadow-lg flex items-center justify-between animate-pulse">
                        <div className="flex items-center gap-4">
                            <div>
                                <h3 className="font-black text-xl">Î×ÎæÎªÎó ÎöÎæÎûÎº ÎñÎóÎÖÎ£!</h3>
                                <p className="text-sm font-bold opacity-80">ÎöÎáÎùÎö 15% ÎæÎûÎ×Îƒ Î¿ÎÖÎªÎö. ÎöÎíÎ£ Î®Î£ÎÜ ÎæÎùÎÖÎíÎøÎòÎƒ Î×Î®Î×ÎóÎòÎ¬ÎÖ.</p>
                            </div>
                        </div>
                ...
                <div className="flex items-center justify-between mb-8">
                    <h1 className="text-3xl font-bold">ÎöÎóÎÆÎ£Îö Î®Î£ÎÖ</h1>
                    {cartItems.length > 0 && (
                        <div className="flex items-center gap-2">
                            <span className="text-black font-normal">Î®ÎÖÎ¬ÎòÎú ÎöÎíÎ£</span>
                            <button
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <div className="flex-1 space-y-6">
                            ...
                            <div key={`${item.id}-${item.size}`} className={`flex items-center gap-4 border p-4 rounded-lg bg-white shadow-sm relative ${item.isPrize ? 'border-amber-300 bg-amber-50' : ''}`}>
                                <div className="w-20 h-20 bg-white flex items-center justify-center text-2xl rounded overflow-hidden relative border border-gray-100 flex-shrink-0">
                                    {item.image_url ? (
                                        ...
                                        <span>{item.isPrize ? '­ƒÄü' : '­ƒº┤'}</span>
                                    )}
                                </div>

                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold truncate">{item.name}</h3>
                                    <div className="text-sm text-gray-500">ÎÆÎòÎôÎ£: {item.size === 'set' ? 'ÎíÎÿ' : `${String(item.size).replace(/ml$/i, '')} Î×"Î£`}</div>
                                    <div className={`text-sm font-bold mt-1 ${item.isPrize ? 'text-green-600' : 'text-primary'}`}>{item.price} Ôé¬ {item.isPrize && '(ÎñÎ¿Îí)'}</div>
                                </div>
                        ...
                        {vendorBuckets.length > 1 && (
                            <div className="p-6 bg-blue-50 border border-blue-100 rounded-2xl flex items-center gap-4 text-blue-700">
                                <div className="text-3xl">­ƒÄÆ</div>
                                <div>
                                    <p className="font-black text-lg">Î®ÎÉÎ¿ ÎöÎñÎ¿ÎÖÎÿÎÖÎØ Î×ÎùÎøÎÖÎØ Î£ÎÜ!</p>
                                    <p className="text-sm">Î£ÎÉÎùÎ¿ Î®Î¬Î®Î£ÎÖÎØ ÎÉÎ¬ ÎöÎöÎûÎ×ÎáÎö Î×-{activeItems[0]?.vendorName || 'ÎíÎñÎº ÎûÎö'} Î¬ÎòÎøÎ£ Î£ÎóÎæÎòÎ¿ Î£ÎíÎ£ ÎöÎæÎÉ.</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="w-full lg:w-96 space-y-6">
                        <div className="bg-white p-6 rounded-xl border shadow-xl space-y-6 sticky top-24">
                            <h2 className="text-xl font-bold border-b pb-4">ÎíÎÖÎøÎòÎØ ÎöÎûÎ×ÎáÎö</h2>

                            {/* Subtotal */}
                            <div className="flex justify-between items-center text-lg">
                                <span>ÎíÎøÎòÎØ ÎæÎÖÎáÎÖÎÖÎØ</span>
                                <div className="flex items-center gap-2">
                                    <span className="font-bold whitespace-nowrap">{subtotal} Ôé¬</span>
                                    <span className="text-sm text-gray-500 font-normal whitespace-nowrap" dir="rtl">({totalItemsCount} {totalItemsCount === 1 ? 'ÎñÎ¿ÎÖÎÿ' : 'ÎñÎ¿ÎÖÎÿÎÖÎØ'})</span>
                                </div>
                            </div>
                            ...
                            {isMainVendor && luckyPrize?.type === 'discount' && !lotteryMode?.active && (
                                <div className="flex justify-between text-lg text-green-600 font-bold">
                                    <span>ÎöÎáÎùÎ¬ ÎÆÎ£ÎÆÎ£ ÎöÎ×ÎûÎ£ ({luckyPrize.value * 100}%)</span>
                                    <span>{Math.round(subtotal * luckyPrize.value)}- Ôé¬</span>
                                </div>
                            )}
                            ...
                            {isMainVendor && coupon && (
                                <div className="flex justify-between text-lg text-green-600 font-bold">
                                    <span>ÎºÎòÎñÎòÎƒ {coupon.code} ({coupon.discountPercent}%)</span>
                                    <span>
                                        {(() => {
                                            if (!coupon) return 0;
                                            let limits = coupon.limitations || {};
                            ...
                            <div className="space-y-4">
                                    ...
                                    <div className="border-t border-b py-4">
                                        {coupon ? (
                                            <div className="flex justify-between items-center bg-green-50 p-3 rounded border border-green-200">
                                                <div>
                                                    <div className="font-bold text-green-700">ÎºÎòÎñÎòÎƒ {coupon.code}</div>
                                                    <div className="text-xs text-green-600">ÎöÎáÎùÎö Î®Î£ {coupon.discountPercent}%</div>
                                                </div>
                                                <button onClick={() => setCoupon(null)} className="text-red-500 hover:bg-red-50 p-1 rounded">Ô£ò</button>
                                            </div>
                                        ) : (
                                ...
                                {(isMainVendor || vendorConfig?.delivery_active || vendorConfig?.self_pickup_active) && (
                                    <div className="space-y-2">
                                        <p className="text-xs font-bold text-gray-500 uppercase tracking-widest text-right">Î®ÎÖÎÿÎ¬ ÎÉÎíÎñÎºÎö</p>
                                        <div className="grid grid-cols-2 gap-3">
                                            {(isMainVendor || vendorConfig?.delivery_active) && (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-xs font-bold leading-tight">Î×Î®Î£ÎòÎù</span>
                                                    <span className={`text-xs font-bold ${!isSelfPickup ? 'text-gray-300' : 'text-gray-400'}`}>{isMainVendor ? 30 : (vendorConfig?.delivery_price || 0)} Ôé¬</span>
                                                </button>
                                            )}
                                            {(isMainVendor || vendorConfig?.self_pickup_active) && (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-xs font-bold leading-tight">ÎÉÎÖÎíÎòÎú ÎóÎªÎ×ÎÖ</span>
                                                    <span className={`text-xs font-bold ${isSelfPickup ? 'text-green-400' : 'text-green-600'}`}>ÎùÎÖÎáÎØ</span>
                                                </button>
                                            )}
                                        </div>
                                        {isSelfPickup && (
                                            <p className="text-xs text-gray-500 text-center pt-1">ÎøÎ¬ÎòÎæÎ¬ ÎÉÎÖÎíÎòÎú ÔÇö ÎÖÎªÎòÎ¿Îú ÎñÎ¿ÎÿÎÖÎØ ÎóÎØ ÎÉÎÖÎ®ÎòÎ¿ ÎöÎöÎûÎ×ÎáÎö</p>
                                        )}
                                    </div>
                                )}
                                ...
                                {!isMainVendor && freeSamplesCount > 0 && (
                                    <div className="bg-green-50 border border-green-100 p-4 rounded-xl flex items-center gap-3 animate-bounce">
                                        <div className="bg-green-600 text-white w-10 h-10 rounded-full flex items-center justify-center text-xl">­ƒÄü</div>
                                        <div>
                                            <p className="text-green-900 font-bold text-sm">ÎöÎÿÎæÎö Î×ÎùÎøÎö Î£ÎÜ!</p>
                                            <p className="text-green-700 text-xs font-medium">Î×ÎÆÎÖÎó Î£ÎÜ {freeSamplesCount} ÎôÎòÎÆÎ×ÎÖÎòÎ¬ ÎùÎÖÎáÎØ ÎæÎöÎûÎ×ÎáÎö ÎûÎò!</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {isMainVendor ? (
                                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                                        <h3 className="font-bold text-gray-900 mb-2 flex items-center gap-2 text-right justify-start">
                                            <span>­ƒÄü</span>
                                            {freeSamplesCount === 6 ? (
                                                <span className="text-green-600">ÎºÎÖÎæÎ£Î¬ ÎÉÎ¬ ÎøÎ£ ÎöÎôÎòÎÆÎ×ÎÖÎòÎ¬! (6)</span>
                                            ) : (
                                                <span>ÎôÎòÎÆÎ×ÎÖÎòÎ¬ ÎùÎÖÎáÎØ</span>
                                            )}
                                        </h3>
                                        ...
                                        <div className="relative h-4 bg-gray-100 rounded-full overflow-hidden mb-2">
                                            ...
                                            ></div>
                                            <div className="absolute top-0 right-[30%] h-full w-0.5 bg-white/50 z-10" title="300Ôé¬ - 2 ÎôÎòÎÆÎ×ÎÖÎòÎ¬"></div>
                                            <div className="absolute top-0 right-[50%] h-full w-0.5 bg-white/50 z-10" title="500Ôé¬ - 4 ÎôÎòÎÆÎ×ÎÖÎòÎ¬"></div>
                                        </div>
                                        ...
                                        <div className="mt-3 text-xs text-center font-bold">
                                            {freeSamplesCount === 0 && <span className="text-gray-500">ÎóÎòÎô <span className="font-bold text-black">{300 - subtotal} Ôé¬</span> Î£-2 ÎôÎòÎÆÎ×ÎÖÎòÎ¬ ÎùÎÖÎáÎØ!</span>}
                                            {freeSamplesCount === 2 && <span className="text-blue-600">ÎÖÎ® Î£ÎÜ 2 ÎôÎòÎÆÎ×ÎÖÎòÎ¬. ÎóÎòÎô <span className="font-bold">{500 - subtotal} Ôé¬</span> Î£-4 ÎôÎòÎÆÎ×ÎÖÎòÎ¬!</span>}
                                            {freeSamplesCount === 4 && <span className="text-purple-600">ÎòÎòÎÉÎò! 4 ÎôÎòÎÆÎ×ÎÖÎòÎ¬ Î®Î£ÎÜ. ÎóÎòÎô <span className="font-bold">{1000 - subtotal} Ôé¬</span> Î£-6 ÎôÎòÎÆÎ×ÎÖÎòÎ¬!</span>}
                                            {freeSamplesCount === 6 && <span className="text-green-600 font-bold">ÎñÎÖÎáÎºÎáÎò ÎÉÎòÎ¬ÎÜ Îæ-6 ÎôÎòÎÆÎ×ÎÖÎòÎ¬! Î¬ÎöÎáÎö! ­ƒÄë</span>}
                                        </div>
                                    </div>
                                    ...
                                    return (
                                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                                            <h3 className="font-bold text-gray-900 mb-2 flex items-center gap-2 text-right justify-start">
                                                <span>­ƒÄü</span>
                                                {currentTier ? (
                                                    <span className="text-green-600">ÎºÎÖÎæÎ£Î¬ {currentTier.samplesCount} ÎôÎòÎÆÎ×ÎÖÎòÎ¬ ÎùÎÖÎáÎØ!</span>
                                                ) : (
                                                    <span>ÎôÎòÎÆÎ×ÎÖÎòÎ¬ ÎùÎÖÎáÎØ</span>
                                                )}
                                            </h3>
                                            ...
                                            <div className="relative h-4 bg-gray-100 rounded-full overflow-hidden mb-2">
                                                ...
                                                {sortedTiers.map((t, idx) => (
                                                    <div
                                                        ...
                                                        title={`${t.minAmount}Ôé¬ - ${t.samplesCount} ÎôÎòÎÆÎ×ÎÖÎòÎ¬`}
                                                    ></div>
                                                ))}
                                            </div>
                                            ...
                                            <div className="mt-3 text-xs text-center font-bold">
                                                {nextTierObj ? (
                                                    <span className="text-emerald-600">
                                                        {currentTier ? `ÎÖÎ® Î£ÎÜ ${currentTier.samplesCount} ÎôÎòÎÆÎ×ÎÖÎòÎ¬. ` : ""}
                                                        ÎóÎòÎô <span className="font-bold text-black">{nextTierObj.minAmount - subtotal} Ôé¬</span> Î£-{nextTierObj.samplesCount} ÎôÎòÎÆÎ×ÎÖÎòÎ¬!
                                                    </span>
                                                ) : (
                                                    <span className="text-green-600 font-bold">ÎñÎÖÎáÎºÎáÎò ÎÉÎòÎ¬ÎÜ Îæ-{currentTier?.samplesCount} ÎôÎòÎÆÎ×ÎÖÎòÎ¬! Î¬ÎöÎáÎö! ­ƒÄë</span>
                                                )}
                                            </div>
                                        </div>
                                    );
                                ...
                                {/* Recommendations / Upsell */}
                                 {recommendations.length > 0 && (
                                     <div className="space-y-3 pt-2">
                                         <h4 className="text-sm font-bold text-gray-700">ÎöÎ®Î£ÎØ ÎÉÎ¬ ÎöÎùÎíÎ¿ ÎæÎºÎ£ÎòÎ¬:</h4>
                                         <div className="space-y-2">
                                             {recommendations.map(rec => (
                                                 <div key={rec.id} className="flex items-center gap-3 bg-white border p-2 rounded-lg shadow-sm hover:shadow-md transition">
                                                     <div className="w-10 h-10 bg-gray-50 rounded flex-shrink-0 flex items-center justify-center overflow-hidden">
                                                         {rec.image_url ? <img src={rec.image_url} alt="" className="w-full h-full object-contain p-1" /> : '­ƒº┤'}
                                                     </div>
                                                     <div className="flex-1 min-w-0">
                                                         <div className="font-bold text-xs truncate">{rec.name}</div>
                                                         <div className="text-xs text-gray-500">{rec.size} Î×"Î£ ÔÇó {rec.price} Ôé¬</div>
                                                     </div>
                                                     <button
                            ...
                            <div className="flex justify-between text-xl font-bold pt-4 border-t">
                                <span>ÎíÎöÎ┤Îø Î£Î¬Î®Î£ÎòÎØ</span>
                                <span>{effectiveTotal} Ôé¬</span>
                            </div>

                            {/* Order Notes */}
                            <div className="py-2">
                                <label className="text-sm font-bold text-gray-700 mb-2 block">ÎöÎóÎ¿ÎòÎ¬ Î£ÎöÎûÎ×ÎáÎö (ÎÉÎòÎñÎªÎÖÎòÎáÎ£ÎÖ):</label>
                                <textarea
                                    className="w-full p-3 border rounded-lg text-sm focus:ring-2 focus:ring-gray-900 outline-none resize-none bg-white"
                                    rows="3"
                                    placeholder="ÎæÎºÎ®ÎòÎ¬ Î×ÎÖÎòÎùÎôÎòÎ¬ Î£ÎÖÎ×ÎÖ ÎöÎòÎ£ÎôÎ¬ / ÎÉÎ¿ÎÖÎûÎö / Î®Î£ÎÖÎù..."
                                    value={notes}
                            ...
                            <div className="py-2 border-t pt-4">
                                ...
                                <p className="text-[10px] text-gray-400 mt-1">ÎáÎ®Î¬Î×Î® ÎæÎ×ÎíÎñÎ¿ ÎûÎö Î¿Îº Î£Î¬ÎÖÎÉÎòÎØ ÎöÎ×Î®Î£ÎòÎù ÎòÎöÎ¬Î®Î£ÎòÎØ.</p>
                            </div>
                        </div>
                        ...
                        <div className="pt-4 sticky top-[calc(6rem+400px)]">
                            {isSignedIn ? (
                                ...
                                >
                                    {isSubmitting ? 'Î×ÎóÎæÎô...' : 'ÎÖÎªÎÖÎ¿Î¬ ÎöÎûÎ×ÎáÎö'}
                                </button>
                            ) : (
                                <SignInButton mode="modal">
                                    <button className="w-full py-4 text-xl font-bold text-white bg-black rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                                        ÎöÎ¬ÎùÎæÎ¿ ÎøÎôÎÖ Î£ÎöÎûÎ×ÎÖÎƒ
                                    </button>
                                </SignInButton>
                            )}
                            <p className="text-xs text-center text-gray-500 mt-2">
                                * ÎöÎ¬Î®Î£ÎòÎØ Î×Î¬ÎæÎªÎó Î×ÎòÎ£ ÎáÎªÎÖÎÆ Î£ÎÉÎùÎ¿ ÎÉÎÖÎ®ÎòÎ¿ ÎöÎöÎûÎ×ÎáÎö
                            </p>
                        </div>
                    </div>
                </div>
