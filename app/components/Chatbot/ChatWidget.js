...
export default function ChatWidget() {
    ...
    const getBotResponse = async (text) => {
        ...
        // 2. Recommendations / General Help (Rich UI Response)
        ...
        // 3. Product Search Intent (Dynamic API)
        ...
        // 4. Fallback -> Instagram/WhatsApp Handoff
        return {
            id: Date.now(),
            text: "הממ... שאלה טובה. 🤔\nבוא/י נעבור לאינסטגרם או לוואטסאפ, שם אוכל לתת לך מענה מהיר ומדויק יותר!",
            sender: 'bot',
            type: 'fallback_instagram'
        };
    };
                        ...
                        {messages.map((msg) => (
                            <div key={msg.id} className={`mb-2 flex ${msg.sender === 'bot' ? 'justify-start' : 'justify-end'}`}>
                                <div className={`max-w-[85%] rounded-2xl p-3 text-sm shadow-sm ${msg.sender === 'bot'
                                        ...
                                        <div className="mt-4 flex flex-wrap gap-2 justify-center">
                                            {msg.options.map((opt, idx) => (
                                                <button
                                                    ...
                                                    className="bg-transparent border border-transparent hover:border-gray-300 text-gray-500 hover:text-black text-xs px-3 py-1.5 rounded-full transition-all duration-300"
                                                >
                                                    {opt}
                                                </button>
                                            ))}
                                    ...
                                    {/* Returns Options (Instagram + Contact) */}
                                    ...
                                    {msg.type === 'fallback_instagram' && (
                                        <div className="mt-3 flex flex-col gap-2">
                                            ...
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="flex items-center justify-center gap-2 bg-green-500 text-white py-2 px-4 rounded-full text-xs font-bold hover:bg-green-600 transition transform hover:scale-105"
                                            >
