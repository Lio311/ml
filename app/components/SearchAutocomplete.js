    ...
    return (
        ...
        <div className={`relative group ${fullWidth ? 'flex-1' : 'inline-block'}`} ref={wrapperRef}>
            <form onSubmit={handleSubmit} className="relative flex items-center w-full">
                ...
                {/* Search Icon (Always on the LEFT as requested) */}
                <button
                    type="submit"
                    className={`absolute top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-black hover:text-black p-1 ${isRTL ? 'end-0' : 'start-0'}`}
                >
