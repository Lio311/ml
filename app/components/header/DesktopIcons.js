...
export default function DesktopIcons({ cartCount, wishlistCount, onSearchToggle, hideSearch = false, enableCatalogs = true }) {
    const { t } = useLanguage();
    const { setIsMiniCartOpen } = useCart();
    ...
    return (
        <div className="flex items-center justify-end gap-2 xl:gap-3 pe-1 md:pe-2">
            ...
            <SignedIn>
                {enableCatalogs && (
                    <Link href="/my-catalogs" className="relative group text-black hidden md:block" title={t('common.manage_catalogs')}>
