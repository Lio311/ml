...
export const sendEmail = async (to, subject, html, type = 'system', orderId = null, campaignId = null, attachments = [], skipShabbatCheck = false, replyTo = null) => {
    ...
    try {
        ...
        const isMarketing = ['manual_campaign', 'marketing', 'recommendations', 'new_product', 'new_preorder', 'review_request', 'cart_recovery', 'educational', 'nurture_10_days', 'nurture_25_days', 'discovery_launch', 'new_perfumes_batch', 'monthly_discovery', 'new_discovery_sets'].includes(type) || campaignId !== null;
 ...
 * Supports {{key}} syntax.
 */
export const getNewPreorderTemplate = (data) => {
    return `
    <div dir="rtl" style="font-family: 'Open Sans', 'Open Sans Hebrew', Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; line-height: 1.6;">
        <div style="background-color: #fff; padding: 40px 30px; border-radius: 24px; border: 1px solid #e0f2fe; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
            ...
            <div style="text-align: center; margin-bottom: 30px;">
                <img src="${data.image_url || data.imageUrl || 'https://www.ml-tlv.com/api/assets/logo?type=logo_email'}" alt="${data.model || ''}" style="width: 250px; height: 250px; object-fit: contain; margin: 0 auto;" />
            </div>
        ...
        <div style="text-align: center; padding-top: 20px; padding-bottom: 0; color: #ccc; font-size: 11px;">ml - יוקרה בחתיכות קטנות</div>
    </div>
    `;
};
...
export const getBatchPerfumeItemsHtml = (products) => {
    ...
    return products.map(product => {
        const imageUrl = product.image_url || product.imageUrl || 'https://www.ml-tlv.com/api/assets/logo?type=logo_email';
...
export const getBatchPerfumeTemplate = (products) => {
    const itemsHtml = products ? getBatchPerfumeItemsHtml(products) : '{{itemsHtml}}';
    return `
        <div dir="rtl" style="font-family: 'Open Sans', 'Open Sans Hebrew', Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; line-height: 1.6;">
...
export const getDiscoveryBatchTemplate = (products) => {
    const itemsHtml = products ? getDiscoveryBatchItemsHtml(products) : '{{itemsHtml}}';
    return `
        <div dir="rtl" style="font-family: 'Open Sans', 'Open Sans Hebrew', Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; line-height: 1.6;">
            <div style="background-color: #fff; padding: 30px; border-radius: 24px; border: 1px solid #f0f0f0; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
    ...
    return items.map(item => `
        <li style="margin-bottom: 12px; border-bottom: 1px solid #f0f0f0; padding-bottom: 12px; display: table; width: 100%;">
            ...
            <div style="display: table-cell; vertical-align: middle;">
                ...
                <div style="font-size: 12px; color: #666;"><span dir="ltr" style="display: inline-block;">${item.is_discovery_set ? formatDiscoverySize(item.volume_label) : `${(item.size || '').toString().trim()}ml`} x${item.quantity || 1}</span></div>
            </div>
        </li>
    `).join('');
};
    ...
    return `
        <div dir="rtl" style="font-family: 'Open Sans', 'Open Sans Hebrew', Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; line-height: 1.6;">
            ...
            <div style="background-color: #fff; padding: 30px; border-radius: 24px; border: 1px solid #f0f0f0; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
                ...
                <div style="text-align: center; margin: 30px 0;">
                    <img src="${getAbsoluteImageUrl(product.imageUrl || product.image_url) || 'https://www.ml-tlv.com/api/assets/logo?type=logo_email'}" alt="${product.brand} ${product.model}" style="max-width: 220px; height: auto; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); background-color: #ffffff; background-image: linear-gradient(#ffffff, #ffffff); padding: 10px;" />
                </div>
...
{{bonusText}}
        ...
        'daily_summary': {
            subject: 'סיכום אימיילים יומי - {{date}}',
            content_html: `
        <div dir="rtl" style="font-family: 'Open Sans', 'Open Sans Hebrew', Arial, sans-serif; color: #333; max-width: 700px; margin: 0 auto; line-height: 1.6;">
            <div style="background-color: #fff; padding: 30px; border-radius: 24px; border: 1px solid #f0f0f0; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
                ...
                <div style="margin-top: 30px; text-align: center; padding-top: 20px; border-top: 1px dashed #eaeaea;">
                    <p style="font-size: 14px; font-weight: bold; margin: 0;">סה"כ הודעות: <span style="color: #0284c7;">{{totalCount}}</span></p>
                </div>
            </div>
            <div style="text-align: center; padding-top: 15px; padding-bottom: 0; color: #ccc; font-size: 11px;">
