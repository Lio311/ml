    ...
    try {
        await withClient(async (client) => {
            const [
                ...
                client.query(`SELECT id, brand, model, price_2ml, price_5ml, price_10ml, image_url, category, in_lottery, slug, description, stock, brand_he, model_he, original_size, created_at, discount_percentage, discount_sizes, discount_end_date, is_discovery_set, is_preorder, single_price, volume_label FROM products WHERE active = true AND stock > 0 AND show_on_home = true ORDER BY created_at DESC LIMIT 6`).catch(e => { console.error(e); return { rows: [] }; }),
