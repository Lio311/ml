    ...
    try {
        await withClient(async (client) => {
            const [
                ...
                client.query('SELECT COUNT(DISTINCT brand) FROM products WHERE active = true AND stock > 0 AND (is_discovery_set IS NULL OR is_discovery_set = false)').catch(e => { console.error(e); return { rows: [{ count: 0 }] }; }),
