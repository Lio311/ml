...
// ... imports
    ...
    useEffect(() => {
        if (subtotal >= 1000 && !luckyPrize && !hasSeenWheel) {
                            // Check if we should show it immediately or wait? 
                            // Let's show it.
                            setShowWheel(true);
        }
    }, [subtotal, luckyPrize, hasSeenWheel]);
    ...
    const handleWin = (prize) => {
                        ...
                        // If it's an item, add to cart?
                        if (prize.type === 'item') {
            // Need a dummy product object or ID.
            // Using a unique negative ID or string ID for prizes to avoid collision with DB ids (usually integers).
            const prizeProduct = {
                            id: `prize-${prize.size}`,
                        name: prize.name,
                        image_url: null, // or a gift icon
        ...
        // If discount, it's handled in Context via total calc
        ...
        setTimeout(() => {
                        ...
                        // ... In JSX, above return ... if (cartItems.length === 0) ...

                        // ... In main JSX ...
                        // Add logic to display discount line item in summary if distinct
