...
export async function generateMetadata(props) {
    try {
        ...
        const ogImageUrl = `${baseUrl}/api/og/product?brand=${encodeURIComponent(brandStr)}&model=${encodeURIComponent(modelStr)}&p10=${product.price_10ml || ''}&p5=${product.price_5ml || ''}&p2=${product.price_2ml || ''}&img=${encodeURIComponent(product.image_url || '')}&v=12`;
    ...
    try {
        const res = await pool.query(`
            SELECT p.id, p.slug, p.brand, p.brand_he, p.model, p.model_he, p.name, p.name_he, p.name_en, 
                   ...
                   p.is_discovery_set, p.single_price, p.volume_label, p.volume_label_en,
