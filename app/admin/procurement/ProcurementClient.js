        ...
        <div className="space-y-8 pb-12 overflow-x-hidden" dir="rtl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-4 md:px-0">
                <KPICard 
                    ...
                    value={<span dir="ltr">₪ {Math.round(data?.meta?.totalProfit || 0).toLocaleString()}</span>} 
                    icon={<DollarSign className="text-blue-500" />}
                    trend="+12.5%"
                />
                <KPICard 
                    title="מחזור מכירות" 
                    value={<span dir="ltr">₪ {Math.round(data?.meta?.totalRevenue || 0).toLocaleString()}</span>} 
                                        ...
                                        if (active && payload && payload.length) {
                                            ...
                                            return (
                                                <div className="bg-black/90 text-white p-3 rounded-xl shadow-2xl border border-white/10 backdrop-blur-md text-right" dir="rtl">
                                                    ...
                                                    <p className="text-xs">רווח: <span dir="ltr">₪ {Math.round(item.y).toLocaleString()}</span></p>
