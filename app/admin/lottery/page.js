...
export default function LotteryAdminPage() {
    ...
    const setupDB = async () => {
        if (!confirm("פעולה זו תוודא שהמסד תקין ותכניס את כל המוצרים למאגר. להמשיך?")) return;
        try {
            const res = await fetch('/api/setup-lottery-db');
            const data = await res.json();
            if (data.success) {
                alert("המסד תוקן ועודכן בהצלחה! הרענן את העמוד.");
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold">ניהול מאגר הגרלות (Lottery Pool)</h1>
                <button
                    onClick={setupDB}
                    className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded font-bold shadow flex items-center gap-2"
                >
                    🛠️ תקן/סנכרן מסד נתונים
                </button>
            </div>
