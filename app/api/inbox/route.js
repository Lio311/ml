    ...
    try {
        ...
        if (asAdmin) {
            ...
            query = await pool.query(`
                SELECT c.id, c.participant1_id, c.participant2_id, c.catalog_id, c.order_id, c.updated_at, 
            ...
            query = await pool.query(`
                ...
                existing_convs AS (
                    SELECT id, participant1_id, participant2_id, catalog_id, order_id, updated_at FROM conversations WHERE participant1_id = $2
                )
                SELECT 
        ...
        if (messageRow.sender_role !== 'admin') {
            try {
                ...
                try {
                    ...
                    console.error("DEBUG: POST /api/inbox - Failed to fetch sender name from Clerk", e);
                }
                ...
                console.error("ERROR: POST /api/inbox - Failed to send admin email notification:", emailErr);
            }
        }
