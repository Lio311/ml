...
export default async function BrandPage(props) {
    ...
    try {
        ...
        const res = await client.query(`
            SELECT p.id, p.brand, p.model, p.price_2ml, p.price_5ml, p.price_10ml, p.image_url, p.category, p.in_lottery, p.slug, p.description, p.stock, p.brand_he, p.model_he, p.original_size, p.created_at, 
                   p.discount_percentage, p.discount_sizes, p.discount_end_date, s.sales_count 
            FROM products p 
