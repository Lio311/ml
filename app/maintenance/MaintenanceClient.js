...
export default function MaintenanceClient({ brand: brandProp }) {
    const { user, isLoaded } = useUser();
    const brandCtx = useBrand();
    const brand = brandProp || brandCtx;
            ...
            <div className="max-w-4xl w-full text-center flex flex-col items-center animate-fade-in-up">
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-24 w-full px-4 mt-4">
                    ...
                    <div className="flex flex-col items-center md:items-start space-y-6 w-full max-w-xs z-20 shrink-0">
                        ...
                        <a href={`https://instagram.com/${brand.instagram}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-pink-600 transition-colors self-center md:self-start">
                ...
                <div className="mt-2 md:mt-4 text-[10px] md:text-xs text-gray-400 pb-1 z-20">
                    &copy; {new Date().getFullYear()} {brand.name}. כל הזכויות שמורות.
                </div>
            </div>
